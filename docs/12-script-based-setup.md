...
To pin setup to a fixed release, replace `latest` with a tag such as `v0.8.0`:

```bash
bash <(curl -fsSL https://github.com/ut-issl/issl-ubuntu-environment-setup/releases/download/v0.8.0/setup.sh)
```
