...
To pin setup to a fixed release, replace `latest` with a tag such as `v0.8.3`:

```bash
bash <(curl -fsSL https://github.com/ut-issl/issl-ubuntu-environment-setup/releases/download/v0.8.3/setup.sh)
```
...
Declining leaves your login shell as it is; a later switch tells you the command that hands it over.
